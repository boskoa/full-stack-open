...
blogsRouter.delete('/:id', async (request, response) => {
  ...
  user.blogs = user.blogs.filter(
    (b) => b.toString() !== blogToRemove.id.toString()
  )
  await user.save()
...
blogsRouter.post('/:id/comments', async (request, response) => {
  ...
  commentedBlog.comments = request.body.comments
  await commentedBlog.save()
  return commentedBlog ? response.json(commentedBlog) : response.status(404).end()
})
